<?php
  ...
  <!-- If GET variable hangup exists, then hangup on that call.
  if(isset($_GET['hangup']))
  {
   $callh = $ci->twilio->request($log_url . "/" . $_GET['hangup'], "POST", array("Status" => "completed"));
  }
...
<script type="text/javascript">
  ...
  <!-- Hide time zone setting which is no longer necessary
  ...
  <table>
    ...
    <?php foreach($log_xml->Calls->Call as $call): ?>
      <tr id="<?=$call->Sid?>">
        ...
        <td><?php if ($call->Status == 'in-progress') echo "<b>";?><?php echo $call->Status;?><?php if ($call->Status == 'in-progress') echo "</b>";?>
        <?php if ($call->Status == 'in-progress') {
        echo " | <a href='?hangup=" . $call->Sid . "'>Hangup</a>";} ?></td> <!-- Adds a Hangup action to the call log -->
      </tr>
    <?php endforeach ?>
    </tbody>
  </table>
